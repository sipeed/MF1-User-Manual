...
| `V1.7`   | `2019.07.16` | 支持112x112图片输入计算特征值<br/>特征值进行了压缩|
| `V2.0`   | `2019.07.19` | 支持MF1模块，活体检测，部分功能不支持|
...
| `IO16` | `record key`     | `short press record` <br/>` long press to clear stored face and board cfg` |
| `IO22` | `relay_act_low`  | `relay output`                         |
| `IO23` | `relay_act_high` | `relay output`                         |
...
| [`lcd_cam_roate`](#屏幕及摄像头旋转)                | 设置`LCD`及`CAMERA`进行旋转     |
| [`lcd_cam_roate_ret`](#屏幕及摄像头旋转结果)        | 设置`LCD`及`CAMERA`进行旋转结果 |
| [`add_uer_by_fea`](#通过特征值添加用户)             | 通过特征值添加用户              |
| [`add_uer_by_fea_ret`](#通过特征值添加用户结果)     | 通过特征值添加用户结果          |
| [`set_brd_uart_pin`](#设置模块通信串口引脚)         | 设置模块通信串口引脚            |
| [`set_brd_uart_pin_ret`](#设置模块通信串口引脚结果) | 设置模块通信串口引脚结果        |
...
```json
{"version": 1,"type": "set_brd_uart_pin","cfg": { "get_cfg": 0,"port_tx": 5, "port_rx": 4, "log_tx": 10, "log_rx": 9 }}
```
...
`cfg`： 模块当前配置，与[设置指令一样](#设置模块通信串口引脚)
